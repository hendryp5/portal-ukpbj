...
class Kisar extends CI_Controller {
	...
	public function index()
	{
        // if($this->m_main->logged_id()){
            ...
            if($this->session->userdata('level')==='PENYEDIA'){
                ...
                $this->load->view('penyedia/p_kisar/v_kisar', $data);
            }else{
                echo "Akses Ditolak!";
                redirect('main');
            }
	...
	public function add()
    {
        // if($this->m_main->logged_id()){
            ...
            if ($validation->run()) {
                $kisar->save();
                $this->session->set_flashdata('success', 'Kritik/Saran anda berhasil kami terima!');
                redirect('penyedia/kisar/add');
            }
    
            $this->load->view("penyedia/p_kisar/v_tambahkisar");
        // }else{
            //jika session belum terdaftar, maka redirect ke halaman login
            // redirect("main");
        // }
    ...
    public function lihat($id_kisar = null)
    {
        // if($this->m_pkisar->logged_id()){
            if (!isset($id_kisar)) redirect('penyedia/p_kisar/v_kisar');
            ...
            $this->load->view("penyedia/p_kisar/v_lihatkisar", $data);
        // }else{
            //jika session belum terdaftar, maka redirect ke halaman login
            // redirect("main");
        // }
