...
class M_ppustakadigital extends CI_Model
{
    // PENYEDIA
    ...
    // BEGIN AMBIL DATA BUKUSAKU
    function pustakadigitalbukusaku(){
        $bukusaku = 'bukusaku';

        $this->db->where('jenis_pustaka',$bukusaku);
        $this->db->order_by("created", "desc");
        $pustakadigitalbukusaku = $this->db->get('tb_pustakadigital');
        
        return $pustakadigitalbukusaku->result();    
    }

    // BEGIN AMBIL DATA PERPRES
    function pustakadigitalperpres(){
        $perpres = 'perpres';

        $this->db->where('jenis_pustaka',$perpres);
        $this->db->order_by("created", "desc");
        $pustakadigitalperpres = $this->db->get('tb_pustakadigital');
        
        return $pustakadigitalperpres->result();    
    }

    // BEGIN AMBIL DATA PERBUP
    function pustakadigitalperbup(){
        $perbup = 'perbup';

        $this->db->where('jenis_pustaka',$perbup);
        $this->db->order_by("created", "desc");
        $pustakadigitalperbup = $this->db->get('tb_pustakadigital');
        
        return $pustakadigitalperbup->result();    
    }

    // BEGIN AMBIL DATA SE
    function pustakadigitalse(){
        $se = 'se';

        $this->db->where('jenis_pustaka',$se);
        $this->db->order_by("created", "desc");
        $pustakadigitalse = $this->db->get('tb_pustakadigital');
        
        return $pustakadigitalse->result();    
    }
}